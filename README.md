...
For the current implementation, the only inputs to calculate NOA is single particle energy $E_{n\mathbf{k}}$ and velocity matrix elements $V_{nmk}$, as well as exciton envelope $A_{cv\mathbf{k}}^\lambda$ and energy $\Omega_\lambda$ if excitonic effect are to be included.
