...
For the current implementation, the only inputs to calculate NOA are single particle energy $E_{n\mathbf{k}}$ and velocity matrix elements $V_{nm\mathbf{k}}$, as well as exciton envelope $A_{cv\mathbf{k}}^\lambda$ and energy $\Omega_\lambda$ if excitonic effects are to be included. We use those files generated by VASP at the moment, although interfacing with other codes is straightforward.  
...
- X. Wang and Y. Yan, Ab initio theory of optical activity in α-quartz in the 𝐺⁡𝑊-Bethe-Salpeter-Equation framework, [Phys. Rev. Lett. 136, 186901](https://journals.aps.org/prl/accepted/10.1103/cvjj-kbck). [Arxiv](https://arxiv.org/abs/2604.05450).
