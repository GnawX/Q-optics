...
For the current implementation, the only inputs to calculate NOA are single particle energy $E_{n\mathbf{k}}$ and velocity matrix elements $V_{nm\mathbf{k}}$, as well as exciton envelope $A_{cv\mathbf{k}}^\lambda$ and energy $\Omega_\lambda$ if excitonic effects are to be included. We use those files generated by VASP at the moment, although interfacing with other codes are straightforward (we have an old interface with QE/Yambo, contact me if you need.).  
**The code has not been thoroughly tested. Use at your own risk! Report any issue or suspicious behavior.**
